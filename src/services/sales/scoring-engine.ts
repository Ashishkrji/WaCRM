...
export async function scoreContact(userId: string, contactId: string): Promise<Record<string, unknown>> {
  ...
  const convProb = await predictConversion(userId, contact, totalScore, signals ?? [])
...
async function predictConversion(userId: string, contact: Record<string, unknown>, score: number, signals: Array<{ signal_type: string }>): Promise<number> {
  try {
    ...
    const provider = await tryGetAIProvider(userId)
    if (!provider) return score * 0.7

    const prompt = `Lead scoring analysis. Lead score: ${score}/100. Contact: ${contact.name}, ${contact.company ?? 'No company'}, ${contact.phone ? 'Has phone' : 'No phone'}. Recent signals (30 days): ${signalSummary || 'none'}. 
Respond with JSON only: {"conversion_probability": 45, "reasoning": "brief 1-line reason"}`
    ...
    console.warn('[AI/scoring] Conversion prediction failed:', err)
    return score * 0.7
  }
}
