...
async function extractAndSaveFacts(
  ...
  try {
    ...
    const currentMemory = await getContactMemory(userId, contactId)
    const mergedFacts = {
      ...(currentMemory?.facts || {}),
      ...extracted
    }
...
export async function dispatchAIReply(
  ...
  const { userId, contactId, conversationId, inboundText, messageType } = input
  ...
  try {
    ...
    if (actionMatch) {
      ...
      try {
        if (actionType === 'BOOK_MEETING') {
          const bookingId = await executeBookMeeting(userId, contactId, conversationId, payload)
          if (bookingId) {
            actionExecuted = true
          ...
          const quoteId = await executeCreateQuote(userId, contactId, conversationId, payload)
          if (quoteId) {
            actionExecuted = true
          ...
          const proposalId = await executeCreateProposal(userId, contactId, conversationId, payload)
          if (proposalId) {
            actionExecuted = true
            const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://crm.example.com'
    ...
    if (intentAnalysis) {
      void updateContactMemory(userId, contactId, {
    ...
    void extractAndSaveFacts(userId, contactId, history, inboundText, replyText).then(async (facts) => {
      if (facts) {
        await evaluateAndCreateLead(userId, contactId, conversationId, facts)
      }
    ...
    void upsertAIConversation(userId, conversationId, provider.name).catch(
      (err) => console.warn('[AI/engine] ai_conversations upsert failed:', err)
    )

    // Maybe generate a summary in MongoDB Atlas
    void maybeGenerateSummary(userId, conversationId).catch(
...
async function handleHumanHandoff(args: {
  ...
  const { userId, conversationId, handoffMessage, reason } = args
